    ...
    task sheet: :environment do     
        ...
        Dir.foreach(importPath) do |sheet|
            if sheet.end_with? ".tsv"
                ...
                headers = CSV.read(filePath, headers: true, encoding: 'utf-8', col_sep: "\t").headers
                ...
                importData = CSV.parse(file, headers: true, encoding: 'utf-8', col_sep: "\t", skip_blanks: true).reject { |row| row.all?(&:nil?) } 
